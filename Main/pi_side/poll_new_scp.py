...
def scp(local_ip, local_user, local_dir, file):
    ...
    # Automatically add the server's host key (this is insecure; use for testing only)
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())

    # Connect to the remote server
    ssh.connect("simon", 22, sys.argv[2])

    # Use SCPClient to securely copy the file
    with SCPClient(ssh.get_transport()) as scp:
        scp.put(sound_dir + sys.argv[2] + "_next_byte.wav", )

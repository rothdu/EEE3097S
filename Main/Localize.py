...
def localize(path1,path2,hyperbola = "False"):
    ...
    # tdoa rpi1
    tdoa_rpi1 = gcc_phat(rpi1_chan_2, rpi1_chan_1, SR, 0.0027)  # top left mic

    # tdoa rpi2
    tdoa_rpi2 = gcc_phat(rpi2_chan_2, rpi2_chan_1, SR, 0.0027)  # top left mic
    ...
    def function(variables):
        (x,y)= variables
        e1 = sqrt((x)**2+(y)**2) - sqrt((x-0.8)**2+(y)**2) - (tdoa_rpi1*constant.speed_of_sound)
        e2 = sqrt((x)**2+(y)**2) - sqrt((x)**2+(y-0.5)**2) - (tdoa_rpi2*constant.speed_of_sound)
        return [e1,e2]
    ...
    if(hyperbola):
        ...
        # testing rpi 1
        h_rpi1_test = sqrt((x)**2+(y)**2) - sqrt((x-0.8)**2+(y-0)**2) - (tdoa_rpi1*constant.speed_of_sound)
        # testing rpi 2
        h_rpi2_test = sqrt((x)**2+(y)**2) - sqrt((x-0)**2+(y-0.5)**2) - (tdoa_rpi2*constant.speed_of_sound)
...
def main():
     x,y = localize("Main/rpi1_next_byte.wav","Main/rpi2_next_byte.wav", False)
     print(x)
     print(y)
